...
TRANSLATIONS = {
    "zh_CN": {
        ...
        "progress.eta_seconds": "（剩余约 {value} 秒）",
        "progress.eta_minutes": "（剩余约 {value} 分钟）",
        ...
        "log.config_load_failed": "读取配置文件失败：{error}。已使用默认设置。",
        "log.setup_complete": "环境设置已完成。",
        ...
        "log.update_no_release": "未找到可用的 GitHub Release（含预发行版）。",
        ...
        "log.worker_process_failed": "Worker 运行过程中出错：{error}",
        "log.worker_exit_code": "Worker 已退出，退出代码：{code}。",
        ...
        "log.command_args_invalid": "无法执行命令：参数包含不支持的字符：{error}。",
    ...
    "en_US": {
        ...
        "log.config_load_failed": "Failed to read the configuration file: {error}. Default settings are being used.",
        ...
        "log.update_no_release": "No GitHub release found (including pre-releases).",
        ...
        "log.worker_process_failed": "Worker failed while running: {error}",
        "log.worker_exit_code": "Worker exited with code {code}.",
        ...
        "log.command_args_invalid": "The command cannot be executed because its arguments contain unsupported characters: {error}.",
...
_RESULT_REASONS = {
    ...
    "Draw by adjudication": "裁定为和棋",
    "Draw by agreement": "双方同意和棋",
    "Draw by timeout vs insufficient mating material": "因超时且对方子力不足，裁定为和棋",
...
_WORKER_LINE_PATTERNS = (
    ...
    (r"^This worker is tainted\.\.\.$", "此 Worker 文件已被修改（未通过验证）……"),
